module Webpacker::Helper
  ...
  private
    ...
    def sources_from_manifest_entries(names, type:)
      names.map { |name| current_webpacker_instance.manifest.lookup!(name, { type: type }, request.variant) }.flatten
    end

    def sources_from_manifest_entrypoints(names, type:)
      names.map { |name| current_webpacker_instance.manifest.lookup_pack_with_chunks!(name, { type: type }, request.variant) }.flatten.uniq
    end

    def resolve_path_to_image(name)
      path = name.starts_with?("media/images/") ? name : "media/images/#{name}"
      asset_path(current_webpacker_instance.manifest.lookup!(path, {}, request.variant))
    rescue
      asset_path(current_webpacker_instance.manifest.lookup!(name, {}, request.variant))
    end
end
